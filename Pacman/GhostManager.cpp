...
void GhostManager::reset(unsigned char i_level, const std::array<Position, 4>& i_ghost_positions)
{
	...
	for (Ghost& ghost : ghosts)
	{
		// the blue ghost is used to get the location of the house and the red ghost to get the location of the exit.
...
{
	if (0 == i_pacman.get_energizer_timer()) //no update of the wave timer when Pacman is energized
	{
		if (0 == wave_timer)
		{
			if (7 > current_wave)
			...
			//I took the rules from the website about how to write pacman
			if (1 == current_wave % 2)
			{
