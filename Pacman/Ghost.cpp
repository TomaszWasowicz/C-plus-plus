...
bool Ghost::pacman_collision(const Position& i_pacman_position) const
{
	// the advanced collision checking algorithm.
...
float Ghost::get_target_distance(unsigned char i_direction)
{
	...
	//moving the ghost in a given direction and calculating the distance to the target
...
void Ghost::draw(bool i_flash, sf::RenderWindow& i_window)
{
	...
	auto body_frame = static_cast<unsigned char>(floor(animation_timer / static_cast<float>(GHOST_ANIMATION_SPEED)));
...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	//I don't consider the way back as an available way.
	...
	{
		//I used rand() because I have figured that I only need randomness, I might use <random> here later ...
	...
	if (1 == move)
	{
		...
		//Warping.
		//When the ghost leaves the map, it is moved to the other side.
	...
	{
		if (0 == frightened_mode) //When the ghost is not frightened and collides with Pacman, Pacman dies.
		{
			i_pacman.set_dead(1);
		}
	...
	else
	{
		...
		else //The chase mode
		{
			...
			case 2: //The blue ghost.
			{
				...
				//sending a vector from the red ghost to the second cell in front of Pacman.
				//Then doubling it.
